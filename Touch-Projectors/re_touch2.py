...
# Works exactly like a tablet: touch anywhere, hold for right click, two fingers for right click!
...
touch_stability_threshold = 5  # pixels of movement allowed while touching
...
def mouse_callback(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN and not calibration_done:
        ...
        if len(calibration_points) == 4:
            ...
            print("  • 👆 Quick touch & release: Left click")
            print("  • 👆 Hold finger in place: Right click (after 0.8s)")
            print("  • 👆👆 Quick double touch: Double click")
            print("  • ✌️ Two fingers: Instant right click")
            print("\n⚙️ Controls:")
            print("  + / - : Adjust hold sensitivity")
            print("  c : Recalibrate")
            print("  ESC : Exit")
            print("="*60)
...
# ============================================
# DETECT TWO FINGERS (for instant right click)
# ============================================

def detect_two_fingers(hand_landmarks):
    """Check if both index and middle fingers are extended (peace sign)"""
...
# ============================================
# CHECK IF FINGER IS STABLE (for touch detection)
# ============================================

def is_finger_stable(current_pos, start_pos, threshold=touch_stability_threshold):
    """Check if finger hasn't moved much since touching started"""
    if current_pos is None or start_pos is None:
        return False
    
    distance = math.sqrt((current_pos[0] - start_pos[0])**2 + 
...
def main():
    ...
    global hold_triggered, last_tap_time, last_tap_position, touch_start_pos
    global right_click_triggered, right_click_cooldown
    global prev_fingertip, fingertip_history
    ...
    while True:
        ...
        if not calibration_done:
            ...
            if current_H is not None:
                ...
                # Check for two-finger gesture first (instant right click)
                two_fingers_detected = False
                two_finger_position = None
                ...
                if results.multi_hand_landmarks:
                    for hand_landmarks in results.multi_hand_landmarks:
                        if detect_two_fingers(hand_landmarks):
                            ...
                            two_finger_position = (x, y)
                            
                            # Draw two-finger indicator
                            cv2.circle(display, two_finger_position, 20, (255, 0, 255), 3)
                            cv2.putText(display, "RIGHT CLICK", (two_finger_position[0]-50, two_finger_position[1]-30),
                                       cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 255), 2)
                            
                            # Trigger instant right click
                            if right_click_cooldown <= 0:
                                screen_x, screen_y = warped_to_screen(two_finger_position)
                                mouse.click(Button.right)
                                print(f"👉 Two-finger right click at ({screen_x}, {screen_y})")
                                right_click_cooldown = 20
                
                # Regular one-finger tracking (only if two fingers not detected)
                if results.multi_hand_landmarks and not two_fingers_detected:
                    for hand_landmarks in results.multi_hand_landmarks:
                        ...
                        if fingertip:
                            ...
                            # ============================================
                            # TOUCH DETECTION - NO ZONE, JUST STABILITY!
                            # ============================================
                            
                            # Check if finger is stable (not moving much)
                            finger_stable = is_finger_stable(fingertip, touch_start_pos)
                            
                            if not is_touching:
                                # Not touching - check if we should start touching
                                if prev_fingertip is not None:
                                    # Calculate movement speed
                                    speed = math.sqrt((fingertip[0] - prev_fingertip[0])**2 + 
                                                    (fingertip[1] - prev_fingertip[1])**2)
                                    
                                    # If finger stops moving, start touching
                                    if speed < 3:  # Very slow movement = intentional touch
                                        is_touching = True
                                        touch_start_time = time.time()
                                        touch_start_pos = fingertip
                                        touch_position = (screen_x, screen_y)
                                        hold_triggered = False
                                        print(f"🖐️ Touch started at ({screen_x}, {screen_y})")
                            
                            else:
                                # Currently touching - check if still stable
                                if finger_stable:
                                    # Still touching - check for hold
                                    ...
                                    # Draw progress arc
                                    cv2.ellipse(display, fingertip, (radius, radius), 0, 0, angle, 
                                               (0, 255, 255), 3)
                                    
                                    cv2.putText(display, f"HOLD: {touch_duration:.1f}s", 
                                              (fingertip[0]-40, fingertip[1]-40),
                                    ...
                                    if touch_duration > hold_threshold and not hold_triggered:
                                        ...
                                        hold_triggered = True
                                        cv2.putText(display, "RIGHT CLICK!", (fingertip[0]-50, fingertip[1]-70),
                                                  cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 255), 2)
                                
                                else:
                                    # Finger moved - touch ended
                                    touch_duration = time.time() - touch_start_time
                                    
                                    # If it was a quick touch (< hold threshold), it's a tap
                                    if touch_duration < hold_threshold and not hold_triggered:
                                        ...
                                        if (current_time - last_tap_time < double_tap_threshold and
                                            ...
                                            cv2.putText(display, "DOUBLE TAP!", (fingertip[0]-60, fingertip[1]-70),
                            ...
                            # Store current fingertip for next frame
                ...
                # Show instructions
                instructions = [
                    "👆 Touch & release: Left click",
                    "👆 Hold in place: Right click (0.8s)",
                    "👆👆 Double touch: Double click",
                    "✌️ Two fingers: Instant right click"
                ]
                ...
                if is_touching:
                    cv2.putText(display, "STATUS: TOUCHING", (10, 60),
                               cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)
                else:
                    cv2.putText(display, "STATUS: READY", (10, 60),
        ...
        if key == 27:  # ESC
            ...
            hold_threshold = min(2.0, hold_threshold + 0.1)
            print(f"Hold threshold: {hold_threshold:.1f}s")
        elif key == ord('-') and calibration_done:
            hold_threshold = max(0.3, hold_threshold - 0.1)
            print(f"Hold threshold: {hold_threshold:.1f}s")
        elif key == ord('c'):
            calibration_points = []
            calibration_done = False
